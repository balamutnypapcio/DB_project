...
/**
 * @brief Slot obsługujący przejscie do strony tworzenia grupy
 */
 ...
 */
void MainWindow::handleCreateGroupButton()
{
    Database& db = Database::getInstance();
    // QString newGropuName = "NowaGrupa";
    // QString newGropuUsers = "alice,bob,carol";

    QString newGropuName = ui->groupNameInput->text();
    QString newGropuUsers = ui->newUsaerInput->toPlainText();

    db.createGroup(newGropuName, currentUserId, newGropuUsers);
...
void MainWindow::loadExpenses()
{
    ...
    for (const auto& expense : expenses) {
        ...
        connect(button, &ExpenseButton::clicked, this, [this, id = expense.id, paidBy = expense.paidByUsername, amount = expense.amount]() {
            handleExpenseClick(id, paidBy, amount);
        });
 ...
 * @param paidBy Po samym expenseID nie da sie stwierdzic kto placil, wiec trzeba przekazac
 *
 * Funkcja wywoływana po kliknięciu przycisku wydatku, obecnie tylko loguje
 * informację o kliknięciu w konsoli debugowania.
 */
void MainWindow::handleExpenseClick(int expenseId, const QString& paidBy, double amount)
{
    // ustawiamy obecny expense
    currentExpenseId = expenseId;

    ui->stackedWidget->setCurrentIndex(4);  // Zakładam, że strona szczegółów ma indeks 6
    //nagłówek
    ui->label_paid_by->setText(paidBy);
    ui->label_amount->setText(QString::number(amount, 'f', 2) + " [$]");

    //zapytanie do bazy o podział wydatków
    Database& db = Database::getInstance();
    auto shares = db.getExpenseShares(expenseId);

    // linia z iloscia osob
    ui->forXUsersLabel->setText("For " + QString::number(shares.size()) + " members ");

    // scroll area z tym kto ma jaka czesc wydatku
    QWidget *scrollContent = new QWidget();
    QVBoxLayout *layout = new QVBoxLayout(scrollContent);

    for (const auto& entry : shares) {
        // Każdy wiersz: poziomy widżet z dwoma labelami
        QWidget *rowWidget = new QWidget();
        QHBoxLayout *rowLayout = new QHBoxLayout(rowWidget);

        QLabel *nameLabel = new QLabel(entry.userName);
        QLabel *amountLabel = new QLabel(QString::number(entry.userShare, 'f', 2) + " [$]");

        rowLayout->addWidget(nameLabel);
        rowLayout->addStretch(); // rozpycha
...
/**
 * @brief Reaguje na klikniecie create group.
 */
...
void MainWindow::on_buttonAddNewExpense_clicked()
{
    ui->stackedWidget->setCurrentWidget(ui->createExpensePage);
}


void MainWindow::handleAddExpenseClick(){

    QString paidBy = ui->paidByTextEdit->toPlainText();
    double amount = ui->amountTextEdit->toPlainText().toDouble();
    QString expenseName = ui->expenseNameTextEdit->toPlainText();
    QString participants = ui->participantsLineEdit->toPlainText();


    Database& db = Database::getInstance();
    db.addExpenseToGroup(
        currentGroupId,                             // groupId
        paidBy,                             // paidById
        amount,                        // kwota
        expenseName,                       // opis
        participants     // udziały jako string
        );
...
void MainWindow::on_buttonCreateExpense_clicked()
{
...
void MainWindow::handleDeleteExpenseClick(){

     Database& db = Database::getInstance();
     db.deleteExpenseById(currentExpenseId);

}


void MainWindow::on_buttonDeleteExpense_clicked()
{
...
void MainWindow::handleDeleteGroup(){

    Database& db = Database::getInstance();
    db.deleteGroupById(currentGroupId);
